...
#include <U8g2lib.h>
#include <U8x8lib.h>
U8G2_ST7920_128X64_1_HW_SPI u8g2(U8G2_R0, /* CS=*/ 10, /* reset=*/ 8); // LCD constructor
/////////////////////////////////////////////////////////////////////
const int inPin = A0;//can change
const float SHUNT_CURRENT = 300.00;//A
const float SHUNT_VOLTAGE = 75.0;// mV
const float CORRECTION_FACTOR = 2.00;


const int ITERATION = 50; //can change (see video)
const float VOLTAGE_REFERENCE = 3300.00;//1.1V
...
///////////////////////////////////////////////////////////////////////////////////////
// LCD_128x64_ST7920 Display functions
void draw(){
  ...
  u8g2.drawStr(2, 13, "Speed:");   
  u8g2.drawStr(70, 28, velocity);   
  u8g2.drawStr(88, 28, "KM/H");
  u8g2.drawStr(2, 46, "SOC:");         
  u8g2.drawStr(96, 61, SOC); 
  u8g2.drawStr(112, 61, "%");
  drawXBattery();
  //drawYBattery();
}
...
void readVelocity()
{
  velocityNum = (average * pi * diameter / 60) * (18/5); // Convert RPM to Speed

  led();

  
  // velocityNum = 30;
  dtostrf(velocityNum, 2, 0, velocity); // Convert number to string
}

void readSOC()
{
  SOCNum = (int)(getCurrent())%100; // temporary value
  //Serial.readBytes(SOC, 3);
  dtostrf(SOCNum, 2, 0, SOC); // Convert number to string
}

void drawXBattery() {
  int batteryW = 30;
  int batteryH = 11;
  int batteryXStart = 58;
  int batteryYStart = 50;
  u8g2.drawRFrame(batteryXStart, batteryYStart, batteryW, batteryH, 3);
  u8g2.drawRBox(batteryXStart + 2, batteryYStart + 2, (SOCNum/100.0)*(batteryW-4), batteryH - 4, 3);
  u8g2.drawBox(batteryXStart + batteryW + 1, batteryYStart + batteryH/2 - 1, 2, 3);
}

void drawYBattery() {
  int batteryW = 11;
  int batteryH = 30;
  int batteryXStart = 50;
  int batteryYStart = 38;
  u8g2.drawRFrame(batteryXStart, batteryYStart, batteryW, batteryH, 3);
  u8g2.drawRBox(batteryXStart + 2, ((batteryYStart+2) + (batteryH-4) - 1) - (SOCNum/100.0)*(batteryH-4), batteryW - 4, (SOCNum/100.0)*(batteryH-4), 3);
  u8g2.drawBox(batteryXStart + batteryW/2 - 1, batteryYStart - 2, 3, 2);
}
...
{   
  ...
  int redbrightness=analogRead(0); 
    int bluebrightness=analogRead(0); 

  
  

  bluebrightness=map(velocityNum,0,60,255,0);
  redbrightness=map(velocityNum,0,60,0,255);
...
//////////////////////////////////////////////////////
float getCurrent() // returns current measurement
{
